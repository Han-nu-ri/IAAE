...
def save_losses(epochs, r_losses, d_losses, g_losses):
    ...
    plt.savefig('iaae_celebA/losses.png')
    plt.close()
...
def save_images(each_epoch, images):
    ...
    for i in range(images.shape[0]):
        ...
        plt.axis('off')
    plt.savefig('iaae_celebA/image_at_epoch_{:04d}.png'.format(each_epoch + 1))
    plt.close()
...
def save_scores_and_print(current_epoch, epochs, r_loss, d_loss, g_loss, precision, recall, fid, inception_score_real,
    ...
    f = open("./logs/iaae_celebA/generative_scores.txt", "a")
...
def main():
    ...
    for i in range(0, epochs):
        ...
        for each_batch in tqdm.tqdm(train_loader):
            ...
            r_loss = update_autoencoder(ae_optimizer, X_train_batch, encoder, decoder, inception_model_score, i, batch_count)
